...
from model.vtitans.vtitans import VTitans, resize_pos_embed
from model.vtitans.ops.modules import MSDeformAttn


class VTitansAdapter(VTitans):
    def __init__(self, pretrain_size=224, conv_inplane=64, n_points=4, deform_num_heads=6,
                 ...
                 deform_ratio=1.0, add_titans_feature=True, use_extra_extractor=True, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.num_block = len(self.layers)
        self.pretrain_size = (pretrain_size, pretrain_size)
        self.interaction_indexes = interaction_indexes
        self.add_titans_feature = add_titans_feature
    ...
    def forward(self, x):
        ...
        for i, layer in enumerate(self.interactions):
            ...
            x, c, mem_weight_residual, value_residual = layer(x, c, mem_weight_residual, value_residual,
...
class InteractionBlock(nn.Module):
    ...
    def forward(self, x, c, mem_weight_residual, value_residual, blocks, deform_inputs1, deform_inputs2, H, W):
        x = self.injector(query=x, reference_points=deform_inputs1[0],
                          feat=c, spatial_shapes=deform_inputs1[1],
                          level_start_index=deform_inputs1[2])
        for idx, blk in enumerate(blocks):
            x, mem_weight_residual, value_residual = blk(x, mem_weight_residual, value_residual)
        ...
        if self.extra_extractors is not None:
            for extractor in self.extra_extractors:
                c = extractor(query=c, reference_points=deform_inputs2[0],
                              feat=x, spatial_shapes=deform_inputs2[1],
                              level_start_index=deform_inputs2[2], H=H, W=W)
        return x, c, mem_weight_residual, value_residual
